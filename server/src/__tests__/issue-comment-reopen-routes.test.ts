...
describe.sequential("issue comment reopen routes", () => {
  ...
  it("allows a same-company CEO to append a plain comment to another agent's terminal issue without mutating it", async () => {
    ...
    mockAccessService.decide.mockImplementation(async (input: { action?: string }) => ({
      allowed: input.action === "issue:comment_terminal_ceo",
      action: input.action,
  ...
  it("does not use terminal CEO comment authority for resume attempts", async () => {
    mockIssueService.getById.mockResolvedValue(makeIssue("done"));
    mockAccessService.decide.mockImplementation(async (input: { action?: string }) => ({
    ...
    const res = await request(await installActor(createApp(), {
      ...
      .post("/api/issues/11111111-1111-4111-8111-111111111111/comments")
      .send({ body: "Resume another agent's work.", resume: true });
