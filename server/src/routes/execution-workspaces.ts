...
const ARTIFACT_CLEANUP_CLOSE_ACTIONS = new Set<ExecutionWorkspaceCloseActionKind>([
  "cleanup_command",
  "teardown_command",
  "git_worktree_remove",
  "git_branch_delete",
...
function closeReadinessIncludesArtifactCleanup(readiness: ExecutionWorkspaceCloseReadiness) {
  return readiness.plannedActions.some((action) => ARTIFACT_CLEANUP_CLOSE_ACTIONS.has(action.kind));
}
  ...
  router.patch("/execution-workspaces/:id", validate(updateExecutionWorkspaceSchema), async (req, res) => {
    ...
    if (req.body.status === "archived" && existing.status !== "archived") {
      ...
      try {
        ...
        const shouldCleanupArtifacts = closeReadinessIncludesArtifactCleanup(readiness);
