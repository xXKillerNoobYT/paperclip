...
`pnpm issue-blockers:audit` scans only direct `blocks` relations for the prohibited stored edges: self, ancestor, descendant, `done`, and `cancelled` blockers. It resolves the database through the same configured migration connection used by local embedded PostgreSQL and explicit `DATABASE_URL` installations. It never adds edges, changes issue hierarchy, statuses, review requirements, device gates, or approval gates. Normal issue writes serialize direct-blocker validation with hierarchy reparenting for the company, and only the internal child-creation recovery path may use its documented immediate-child lifecycle gate.
